  ...
  const changePassword = async (req, res) => {
    try {
      ...
      const id = req.params.id; // Or req.user._id if authenticated
      ...
      if (newPassword !== confirmNewPassword) {
        return errorHelper(res, null, "New password and confirm password do not match", 400);
      }

    if(oldPassword === newPassword){
      return errorHelper(res, null, "New password and Old Password must be different", 400);
    }
  ...
  export { registerAdmin, loginAdmin, changePassword, forgotPassword, resetPassword };
